      ...
      <div className="flex flex-col  sm:flex-row">
        {users.map((user, index) => (
          <div className="my-5 flex-auto " key={index}>
            <button  onClick={() => handleView(user.login)} className="bg-blue-500   hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
              {user.login}
            </button>
          </div>
        ))}
